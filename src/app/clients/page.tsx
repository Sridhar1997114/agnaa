...
export default function ClientLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      const cleanId = loginNumber.trim().toUpperCase();
      const isEnthalpyId = cleanId === 'AGN-001' || 
                           cleanId === 'AGN080426-1001' || 
                           cleanId === 'AGN080426-001';
      
      const isValidPassword = password === 'enthalpy@agnaa' || password === 'entlabs';

      if (isEnthalpyId && isValidPassword) {
        localStorage.setItem('client_session', JSON.stringify({
          client: 'Enthalpy Labs',
          id: cleanId,
          lastLogin: new Date().toISOString()
        }));
        router.push('/clients/enthalpy-labs');
      } else {
