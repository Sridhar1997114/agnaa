...
export default function BrickCalculator() {
  ...
  const handleCalculate = () => {
    ...
    if (l > 0 && h > 0 && t > 0) {
      setResults({ ...calculateBrickwork(l, h, t), l, h, t });
  ...
  return (
    <BaseCalculator
      ...
      description="Estimate the exact number of bricks, cement bags, and sand required to construct a wall to precision."
      ...
      pdfTitle="Brickwork\nEstimate"
      pdfProjectInfo={{ 'DOCUMENT TYPE': 'MATERIAL ESTIMATE', 'MORTAR RATIO': '1:6 STANDARD' }}
      inputsContent={
        <div className="space-y-6">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="number" value={height} onChange={(e) => {setHeight(e.target.value); setIsCalculated(false);}} className="bg-white w-full rounded-xl px-3 py-2 text-base font-black text-[#1C1C72] outline-none border border-gray-200 focus:border-[#7B2DBF] transition-colors" placeholder="e.g. 10" />
            </div>
          </div>
          <div>
            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-1">Wall Thickness (INCHES)</label>
            <select value={thickness} onChange={(e) => {setThickness(e.target.value); setIsCalculated(false);}} className="bg-white w-full rounded-xl px-3 py-2 text-base font-black text-[#1C1C72] outline-none border border-gray-200 focus:border-[#7B2DBF] transition-colors appearance-none">
      ...
      resultsContent={
        <div className="bg-gradient-to-br from-[#1C1C72] to-[#2A1B81] rounded-2xl p-6 text-white shadow-xl relative overflow-hidden h-full min-h-[300px] flex flex-col justify-center">
          ...
          <div className="mb-8">
            ...
            <div className="text-5xl font-black text-white">{fmt(results?.noOfBricks || 0)} <span className="text-sm text-gray-400">PCS</span></div>
          </div>
